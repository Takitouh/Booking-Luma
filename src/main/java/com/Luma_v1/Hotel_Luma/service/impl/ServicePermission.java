...
import com.Luma_v1.Hotel_Luma.mapper.PermissionMapper;
...
public class ServicePermission implements IServicePermission {
    ...
    @Override
    public List<ResponsePermissionDTO> allPermissions() {
        List<ResponsePermissionDTO> permissionResponseDTOS = new ArrayList<>();
    ...
    @Override
    public ResponsePermissionDTO createPermission(CreatePermissionDTO permission) {
        Permission perm = permissionMapper.toEntity(permission);
        perm = permissionRepository.save(perm);
        ResponsePermissionDTO responseDTO = permissionMapper.toDto(perm);
        logger.info("Permission created: {}", perm.getPermissionName());
        return responseDTO;
    }

    @Override
    public List<ResponsePermissionDTO> createPermission(Set<CreatePermissionDTO> permissions) {
        List<Permission> per = new ArrayList<>();
        List<ResponsePermissionDTO> permissionDTOList = new ArrayList<>();
        for (CreatePermissionDTO pe : permissions) {
...
//    @Override
//    public ResponsePermissionDTO updatePermission(PermissionPutDTO permissionPutDTO, Long id) {
...
//    @Override
//    public ResponsePermissionDTO updatePermission(PermissionPatchDTO permissionPatchDTO, Long id) {
//        Permission permissionEntity = permissionRepository.findById(id).orElseThrow(() -> new EntityNotFoundException("Permission with id " + id + " not found"));
//
